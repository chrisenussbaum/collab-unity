...
export default function UserProfile({ currentUser: propCurrentUser, authIsLoading: propAuthIsLoading, setCurrentUser }) {
  ...
  const isOwnProfile = propCurrentUser && (
    (username && propCurrentUser.username === username) || 
    (emailParam && propCurrentUser.email === emailParam) ||
    (!username && !emailParam)
  );
  ...
  const { data: profileData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const isOwner = propCurrentUser && propCurrentUser.email === normalizedUser.email;

      const [createdProjects, collaboratingProjects, followedProjectsData, endorsements, reviews, sharedProjectsData] = await Promise.all([
        base44.entities.Project.filter({ created_by: normalizedUser.email }),
        base44.entities.Project.filter({ collaborator_emails: { $in: [normalizedUser.email] } }),
        normalizedUser.followed_projects && normalizedUser.followed_projects.length > 0
          ? base44.entities.Project.filter({ id: { $in: normalizedUser.followed_projects } })
          : Promise.resolve([]),
        normalizedUser.skills && normalizedUser.skills.length > 0
          ? base44.entities.SkillEndorsement.filter({ user_email: normalizedUser.email })
          : Promise.resolve([]),
        base44.entities.CollaboratorReview.filter({ reviewee_email: normalizedUser.email, is_public: true }, '-created_date'),
        !isOwner && propCurrentUser
          ? Promise.all([
              base44.entities.Project.filter({ collaborator_emails: { $in: [propCurrentUser.email] } }),
              base44.entities.Project.filter({ collaborator_emails: { $in: [normalizedUser.email] } })
            ]).then(([userProjects, targetProjects]) => 
              userProjects.filter(p1 => targetProjects.some(p2 => p2.id === p1.id))
            )
          : Promise.resolve([])
      ]);
      ...
      // Enhance reviews with reviewer profiles
      const reviewerEmails = [...new Set(reviews.map(r => r.reviewer_email))];
      let enhancedReviews = reviews;
      ...
      if (reviewerEmails.length > 0) {
        try {
          ...
          enhancedReviews = reviews.map(review => {
      ...
      return {
        ...
        userProjects: visibleProjects,
        followedProjects: visibleFollowedProjects,
        skillEndorsements: endorsements,
        collaboratorReviews: enhancedReviews,
  ...
  const userProjects = profileData?.userProjects || [];
  const followedProjects = profileData?.followedProjects || [];

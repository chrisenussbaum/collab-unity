...
export default function ProjectDetail({ currentUser: propCurrentUser, authIsLoading }) {
  ...
  const initializeProjectPage = useCallback(async () => {
      ...
      hasInitialized.current = false; // Reset hasInitialized at the start of a new (or retry) initialization
      ...
      setHasError(false);
      setProject(null);
      setPendingInvitation(null); // Reset pending invitation state

      try {
        // Load user first (retained internal state management for current user)
        ...
        if (user && !isOwner && !isExplicitCollaborator) {
          try {
            ...
            if (pendingInvites && pendingInvites.length > 0) {
              ...
              pendingInvite = pendingInvites[0]; // Get the first pending invitation
        ...
        if (user && !isOwner && !isExplicitCollaborator) { // Only fetch if not owner/explicit collaborator already
          try {
            ...
            if (allApplications && allApplications.length > 0) {
              ...
              // Re-check access if an accepted application grants it
              if (currentUserApplication.status === 'accepted' && !isExplicitCollaborator) {
                // If application is accepted, user now has access. This would only happen if they were not already an explicit collaborator.
                // For now, allow access based on accepted application directly.
              }
            }
          } catch (error) {
        ...
        // Load template if project was created from one (still needed if ProjectLearningResources or other components uses it)
        ...
        if (isMounted.current) {
          ...
          if (error.response?.status === 429) {
            ...
            if (currentRetry < 3) {
              ...
              // Exponential backoff for full page retry
              setTimeout(() => {
                if (isMounted.current) {
                  previousProjectIdRef.current = null;
                  hasInitialized.current = false; // Reset for retry
  ...
  useEffect(() => {
    ...
    // Only initialize if we haven't already or if the project ID changed
    if (!hasInitialized.current || previousProjectIdRef.current !== projectId) {

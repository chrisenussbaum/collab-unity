      ...
      <div className="max-w-[980px] mx-auto px-4 grid grid-cols-3 gap-8 text-center">
        {stats.map((s) => (
          <div key={s.label}>
            ...
            <div className="text-sm text-purple-700/60">{s.label}</div>
          </div>
        ))}
      </div>
    </section>
...
export function HowItWorks({ onAuth }) {
  ...
  return (
    <section id="how-it-works" className="py-20 px-4 bg-white text-center border-t border-purple-100">
      <div className="max-w-[980px] mx-auto">
        <p className="text-sm font-semibold text-[#5B47DB] mb-2 uppercase tracking-wide">Process</p>
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#3B2A7D] tracking-tight mb-4">
          Start collaborating in minutes.
        </h2>
        <p className="text-lg text-purple-700/60 mb-14 max-w-[500px] mx-auto">Simple, fast, and extremely effective.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((s, i) => {
            const Icon = s.icon;
            return (
              <div key={i} className="relative">
                <div className={`${s.soft} rounded-3xl p-8 flex flex-col items-center text-center border border-purple-100`}>
                  ...
                  <h3 className="text-lg font-bold text-[#3B2A7D] mb-2">{s.title}</h3>
                  <p className="text-sm text-purple-700/60 leading-relaxed">{s.desc}</p>
                </div>
                {i < 2 && (
                  <div className="hidden md:flex absolute top-1/2 -right-4 -translate-y-1/2 z-10 w-8 h-8 bg-white rounded-full shadow border border-purple-100 items-center justify-center">
  ...
  return (
    <section id="about" className="py-20 px-4 bg-[#F8F7FF]">
      <div className="max-w-[700px] mx-auto text-center">
        ...
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#3B2A7D] tracking-tight mb-6 leading-tight">
          Built for builders who think bigger.
        </h2>
        <p className="text-lg text-purple-700/70 leading-relaxed mb-6">
        ...
        <p className="text-lg text-purple-700/70 leading-relaxed">
      ...
      <div className="max-w-[980px] mx-auto mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {values.map((v, i) => {
          const Icon = v.icon;
          return (
            <div key={i} className="bg-white rounded-2xl p-6 text-center shadow-sm border border-purple-100">
              ...
              <h4 className="font-bold text-[#3B2A7D] mb-1">{v.title}</h4>
              <p className="text-sm text-purple-700/60">{v.desc}</p>
            </div>
          );
        })}
      </div>
  ...
  return (
    <section className="py-20 px-4 bg-gradient-to-b from-[#F8F7FF] to-white text-center">
      <div className="max-w-[620px] mx-auto">
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#3B2A7D] tracking-tight mb-4 leading-tight">
          Ready to build something great?
        </h2>
        <p className="text-lg text-purple-700/70 mb-8">
...
export function FAQ() {
  ...
  return (
    <section id="faq" className="py-20 px-4 bg-white border-t border-purple-100">
      <div className="max-w-[700px] mx-auto">
        <h2 className="text-3xl sm:text-4xl font-bold text-[#3B2A7D] text-center mb-12 tracking-tight">
          Frequently asked questions
        </h2>
        <div className="space-y-1">
          {faqs.map((faq, i) => (
            <details key={i} className="group border-b border-purple-100 py-4">
              <summary className="flex items-center justify-between cursor-pointer list-none">
                <span className="text-base font-medium text-[#3B2A7D]">{faq.q}</span>
                <span className="ml-4 flex-shrink-0 w-5 h-5 text-purple-400 transition-transform group-open:rotate-45">
                  <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                  </svg>
                </span>
              </summary>
              <p className="mt-3 text-sm text-purple-700/60 leading-relaxed">{faq.a}</p>
            </details>
          ))}
        </div>
      </div>

    ...
    <div className="border-b border-gray-100 bg-white">
      ...
      <div className="flex items-center justify-between px-4 pt-3 pb-2">
        <div className="flex items-center gap-2">
          ...
          {completionPct > 0 && (
            <span className="text-xs text-gray-400 hidden sm:inline">{completionPct}% complete</span>
          )}
        </div>
      ...
      <div className="border-t border-gray-100">
        ...
        <button
          ...
          className="w-full flex flex-col gap-2 px-4 py-3 hover:bg-purple-50/50 transition-colors text-left border-b border-gray-100 sm:hidden"
        >
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500">
            <span className="flex items-center gap-1"><Circle className="w-3 h-3 text-gray-300" />{todoTasks} to do</span>
            <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-blue-400" />{inProgTasks} active</span>
            <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-green-500" />{doneTasks} done</span>
          </div>
        </button>
        ...
        <button
          ...
          className="w-full flex flex-col gap-2 px-4 py-3 hover:bg-orange-50/50 transition-colors text-left border-b border-gray-100 sm:hidden"
        >
          <div className="flex items-center gap-1.5">
            <Flag className="w-4 h-4 text-orange-500 flex-shrink-0" />
            <span className="text-sm font-semibold text-gray-700">Milestones</span>
            <span className="ml-auto text-xs text-gray-400">{upcomingMilestones.length} upcoming</span>
          </div>
          {upcomingMilestones.length === 0 ? (
            ...
            <div className="space-y-1">
              {upcomingMilestones.map((m, i) => {
                ...
                return (
                  <div key={m.id || i} className="flex items-center justify-between gap-2 min-w-0">
                    <p className="text-xs text-gray-600 truncate">{m.title || m.name}</p>
                    <span className={`text-xs font-medium flex-shrink-0 ${overdue ? "text-red-500" : days <= 7 ? "text-orange-500" : "text-gray-400"}`}>
        ...
        <button
          ...
          className="w-full flex flex-col gap-2 px-4 py-3 hover:bg-green-50/50 transition-colors text-left sm:hidden"
        >
          <div className="flex items-center gap-1.5">
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="space-y-1">
              {recentActivity.map((log, i) => (
                <div key={log.id || i} className="flex items-start gap-1.5 min-w-0">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-400 flex-shrink-0 mt-1" />
                  <p className="text-xs text-gray-600 leading-tight line-clamp-2">{log.action_description}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="hidden sm:grid sm:grid-cols-3 sm:divide-x sm:divide-gray-100">
          <button
            onClick={() => onSectionClick?.("tasks")}
